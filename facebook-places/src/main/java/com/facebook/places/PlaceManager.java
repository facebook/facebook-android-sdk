...
/**
 * Provides an interface to search and query the Places Graph. Supports querying the end user's'
 * current place, searching nearby places, and fetching place information details.
 */
@Deprecated
public class PlaceManager {
  ...
  public enum LocationError {
    /**
     * The location permissions are denied. The SDK requires permissions {@code
     * "android.permission.ACCESS_FINE_LOCATION"} or {@code
     * "android.permission.ACCESS_COARSE_LOCATION"} to retrieve the current location.
     */
  ...
  /**
   * Callback invoked when a request has been constructed and is ready to be executed. To be used
   * with {@link PlaceManager#newCurrentPlaceRequest(CurrentPlaceRequestParams,
  ...
  public interface OnRequestReadyCallback {
    ...
    /**
     * Method invoked when the provided {@code GraphRequest} is ready to be executed. Set a callback
     * on it to handle the response using {@code setCallback}, and then execute the request.
     *
  ...
  /**
   * Creates a new place search request centered around the current device location. The SDK will
   * retrieve the current device location using {@link android.location.LocationManager}
   *
   * <p>With the Places Graph, you can search for millions of places worldwide and retrieve
   * information like number of checkins, ratings, and addresses all with one request.
   *
   * <p>The specified {@link OnRequestReadyCallback} will be invoked once the request has been
   * generated and is ready to be executed.
   *
   * @param requestParams the request parameters. See {@link PlaceSearchRequestParams}
   * @param callback the {@link OnRequestReadyCallback} invoked when the {@link GraphRequest} has
   *     been generated and is ready to be executed
   */
  public static void newPlaceSearchRequest(
      final PlaceSearchRequestParams requestParams, final OnRequestReadyCallback callback) {
          ...
          public void onLocationPackage(LocationPackage locationPackage) {
            if (locationPackage.locationError == null) {
              GraphRequest graphRequest =
                  newPlaceSearchRequestForLocation(requestParams, locationPackage.location);
  ...
  /**
   * Creates a new place search request centered around the specified location. If the location
   * provided is null, the search will be completed globally. At least a location or a search text
   * must be provided.
   *
   * <p>With the Places Graph, you can search for millions of places worldwide and retrieve
   * information like number of checkins, ratings, and addresses all with one request.
   *
   * <p>Returns a new GraphRequest that is configured to perform a place search.
   *
   * @param requestParams the request parameters. See {@link PlaceSearchRequestParams}
   * @param location the {@link Location} around which to search
   * @return a {@link GraphRequest} that is ready to be executed
   * @throws FacebookException thrown if neither {@code location} nor {@code searchText} is
   *     specified
   */
  public static GraphRequest newPlaceSearchRequestForLocation(
      PlaceSearchRequestParams requestParams, Location location) {
    ...
    return new GraphRequest(
        AccessToken.getCurrentAccessToken(), SEARCH, parameters, HttpMethod.GET);
  }

  /**
   * Creates a new place info request.
   *
   * <p>The Places Graph exposes a rich set of information about places. If the request is
   * authenticated with a user access token, you can also obtain social information such as the
   * number of friends who have liked and checked into the PlaceFields. The specific friends are
   * also available if they have authenticated the app with the user_tagged_places and user_likes
   * permissions.
   *
   * <p>Returns a new {@link GraphRequest} that is configured to perform a place info request.
   *
   * @param requestParams the request parameters, a {@link PlaceInfoRequestParams#getPlaceId()} must
   *     be specified.
   * @return a {@link GraphRequest} that is ready to be executed
  ...
  public static GraphRequest newPlaceInfoRequest(PlaceInfoRequestParams requestParams) {
    ...
    return new GraphRequest(
        AccessToken.getCurrentAccessToken(), placeId, parameters, HttpMethod.GET);
  }

  /**
   * Creates a new current place request.
   *
   * <p>The current place request estimates the place where the user is currently located. The
   * response contains a list of places and their associated confidence levels.
   *
   * <p>If a location is not specified in {@link CurrentPlaceRequestParams}, then the SDK retrieves
   * the current location using {@link android.location.LocationManager}.
   *
   * @param requestParams the request parameters. See {@link CurrentPlaceRequestParams}
   * @param callback a {@link OnRequestReadyCallback} that is invoked when the {@link GraphRequest}
   *     has been created and is ready to be executed.
   */
  public static void newCurrentPlaceRequest(
      final CurrentPlaceRequestParams requestParams, final OnRequestReadyCallback callback) {
   ...
   *
   * <p>This request allows users to provide feedback on the accuracy of the current place estimate.
   * This information is used to improve the accuracy of our results.
   *
   * <p>Returns a new GraphRequest that is configured to perform a current place feedback request.
   *
   * @param requestParams the request parameters. See {@link CurrentPlaceFeedbackRequestParams}
   * @return a {@link GraphRequest} that is ready to be executed
   * @throws FacebookException thrown if parameters {@link
   *     CurrentPlaceFeedbackRequestParams#getPlaceId()}, {@link
   *     CurrentPlaceFeedbackRequestParams#getTracking()}, or {@link
   *     CurrentPlaceFeedbackRequestParams#wasHere()} are missing
   */
  public static GraphRequest newCurrentPlaceFeedbackRequest(
    ...
    return new GraphRequest(
        AccessToken.getCurrentAccessToken(), CURRENT_PLACE_FEEDBACK, parameters, HttpMethod.POST);
  }

  private static Bundle getCurrentPlaceParameters(
      CurrentPlaceRequestParams request, LocationPackage locationPackage) throws FacebookException {
