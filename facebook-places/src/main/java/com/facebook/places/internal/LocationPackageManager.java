...
public class LocationPackageManager {
  ...
  public interface Listener {
    /**
     * Invoked when the location sensor data has been collected. The location sensor data can then
     * be used to increase the accuracy when placing a current place request. {@link PlaceManager}
     *
  ...
  public static void requestLocationPackage(
      final LocationPackageRequestParams requestParams, final Listener listener) {
                ...
                try {
                  ...
                  if (requestParams.isLocationScanEnabled()) {
                    ...
                    LocationScanner locationScanner =
                        ScannerFactory.newLocationScanner(
                            FacebookSdk.getApplicationContext(), requestParams);
  ...
  private static FutureTask<LocationPackage> newLocationScanFuture(
      final LocationScanner locationScanner, final LocationPackageRequestParams requestParams) {
    return new FutureTask<>(
        new Callable<LocationPackage>() {
          @Override
