...
public class ShareGraphRequest {
   ...
   *
   * <p>Use this method to create an open graph object, which can then be posted utilizing the same
   * GraphRequest methods as other GraphRequests.
   *
   * @param openGraphObject The open graph object to create. Only SharePhotos with the imageUrl set
   *     are accepted through this helper method.
   * @return GraphRequest for creating the given openGraphObject
   * @throws com.facebook.FacebookException thrown in the case of a JSONException or in the case of
  ...
  public static GraphRequest createOpenGraphObject(final ShareOpenGraphObject openGraphObject)
    ...
    try {
      JSONObject stagedObject =
          (JSONObject)
              OpenGraphJSONUtility.toJSONValue(
                  openGraphObject,
      ...
      return new GraphRequest(
          AccessToken.getCurrentAccessToken(), graphPath, parameters, HttpMethod.POST);
