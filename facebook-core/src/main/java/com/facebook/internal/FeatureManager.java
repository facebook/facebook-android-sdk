...
public final class FeatureManager {
  ...
  public static void checkFeature(final Feature feature, final Callback callback) {
    FetchedAppGateKeepersManager.loadAppGateKeepersAsync(
        new FetchedAppGateKeepersManager.Callback() {
          @Override
  ...
  private static synchronized void initializeFeatureMapping() {
    ...
    featureMapping.put(
        FeatureManager.Feature.IntelligentIntegrity,
        new String[] {
   ...
   *
   * <p>Sample: AppEvents = 0x00010000, ^ ^ ^ ^ | | | | kit | | | feature | | sub-feature |
  ...
  /**
   * Callback for fetching feature status. Method {@link FeatureManager#checkFeature(Feature,
   * Callback)}} will call GateKeeper manager to load the latest GKs first and then run the callback
   * function.
   */
