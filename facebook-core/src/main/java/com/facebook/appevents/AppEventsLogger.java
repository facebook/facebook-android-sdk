 ...
 * In order to log events, the app must create an instance of this class via a {@link #newLogger
 * newLogger} method, and then call the various "log" methods off of that.
 *
 * <p>This client-side event logging is then available through Facebook App Insights and for use
 * with Facebook Ads conversion tracking and optimization.
 *
 * <p>The AppEventsLogger class has a few related roles:
 *
 * <ul>
 *   <li>Logging predefined and application-defined events to Facebook App Insights with a numeric
 *       value to sum across a large number of events, and an optional set of key/value parameters
 *       that define "segments" for this event (e.g., 'purchaserStatus' : 'frequent', or
 ...
 *   <li>Methods that control the way in which events are flushed out to the Facebook servers.
 * </ul>
 *
 * <p>Here are some important characteristics of the logging mechanism provided by AppEventsLogger:
 *
 * <ul>
 *   <li>Events are not sent immediately when logged. They're cached and flushed out to the Facebook
 *       servers in a number of situations:
 *       <ul>
 *         <li>when an event count threshold is passed (currently 100 logged events).
 *         <li>when a time threshold is passed (currently 15 seconds).
 *         <li>when an app has gone to background and is then brought back to the foreground.
 *       </ul>
 *   <li>Events will be accumulated when the app is in a disconnected state, and sent when the
 *       connection is restored and one of the above 'flush' conditions are met.
 *   <li>The AppEventsLogger class is intended to be used from the thread it was created on.
 *       Multiple AppEventsLoggers may be created on other threads if desired.
 *   <li>The developer can call the setFlushBehavior method to force the flushing of events to only
 ...
 *       by calling FacebookSdk.addLoggingBehavior(LoggingBehavior.APP_EVENTS);
 * </ul>
 *
 * <p>Some things to note when logging events:
 *
 * <ul>
 *   <li>There is a limit on the number of unique event names an app can use, on the order of 1000.
 *   <li>There is a limit to the number of unique parameter names in the provided parameters that
 *       can be used per event, on the order of 25. This is not just for an individual call, but for
 *       all invocations for that eventName.
 *   <li>Event names and parameter names must be between 2 and 40 characters, and must consist of
 *       alphanumeric characters, _, -, or spaces.
 *   <li>The length of each parameter value can be no more than on the order of 100 characters.
 * </ul>
 */
public class AppEventsLogger {
   ...
   * start being logged automatically. By default this function is called automatically from
   * sdkInitialize() flow. In case 'com.facebook.sdk.AutoLogAppEventsEnabled' manifest setting is
   * set to false, it should typically be called from the OnCreate method of you application.
   *
   ...
   * set to false, it should typically be called from the OnCreate method of you application.
   *
   * <p>Call this if you wish to use a different Application ID then the one specified in the
   * Facebook SDK.
   *
   * @param application The running application
  ...
  /**
   * @deprecated When using {@link AppEventsLogger#activateApp(Application)} deactivate app will be
   ...
   * that the app is utilizing. This is called internally and does NOT need to be called externally.
   *
  ...
  /**
   * Build an AppEventsLogger instance to log events through. The Facebook app that these events are
   * targeted at comes from this application's metadata. The application ID used to log events will
   ...
   *     will be used, if any; if not the logging will happen against the default app ID specified
  ...
  public static AppEventsLogger newLogger(
      Context context, String applicationId, AccessToken accessToken) {
    return new AppEventsLogger(context, applicationId, accessToken);
  }

  /**
   * Build an AppEventsLogger instance to log events that are attributed to the application but not
  ...
  private AppEventsLogger(Context context, String applicationId, AccessToken accessToken) {
    loggerImpl = new AppEventsLoggerImpl(context, applicationId, accessToken);
  }

  /**
   * The action used to indicate that a flush of app events has occurred. This should be used as an
   ...
   *     constants are applicable. Event names should be 40 characters or less, alphanumeric, and
   *     can include spaces, underscores or hyphens, but must not have a space or hyphen as the
   *     first character. Any given app should have no more than 1000 distinct event names.
   */
   ...
   *     constants are applicable. Event names should be 40 characters or less, alphanumeric, and
   *     can include spaces, underscores or hyphens, but must not have a space or hyphen as the
   *     first character. Any given app should have no more than 1000 distinct event names. * @param
   *     eventName
   ...
   *     parameters with each distinct eventName. It's advisable to limit the number of unique
   *     values provided for each parameter in the thousands. As an example, don't attempt to
   *     provide a unique parameter value for each unique user in your app. You won't get meaningful
   *     aggregate reporting on so many parameter values. The values in the bundles should be
   ...
   *     no more than 24 entries, and keys should be mostly consistent from one purchase event to
   *     the next.
   */
  ...
  /**
   * @deprecated Use {@link AppEventsLogger#logPurchase( java.math.BigDecimal, java.util.Currency,
   ...
   *     out of stock - No plan to restock preorder - Available in future available for order -
   *     Ships in 1-2 weeks discontinued - Discontinued
  ...
  /**
   * Intended to be used as part of a hybrid webapp. If you call this method, the FB SDK will add a
   * new JavaScript interface into your webview. If the FB Pixel is used within the webview, and
   * references the app ID of this app, then it will detect the presence of this injected JavaScript
   * object and pass Pixel events back to the FB SDK for logging using the AppEvents framework.
   *
  ...
  /**
   * Sets a user id to associate with all app events. This can be used to associate your own user id
   * with the app events logged from this instance of an application.
   *
   * <p>The user ID will be persisted between application instances.
   *
   ...
   *
   * <p>The user data will be persisted between application instances.
   *
