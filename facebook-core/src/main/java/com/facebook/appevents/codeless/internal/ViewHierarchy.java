...
public class ViewHierarchy {
  ...
  public static void updateBasicInfoOfView(View view, JSONObject json) {
    try {
      ...
      if (tag != null) {
        json.put(TAG_KEY, Utility.coerceValueIfNullOrEmpty(Utility.sha256hash(tag.toString()), ""));
      }
      if (description != null) {
        json.put(
            DESC_KEY,
  ...
  public static int getClassTypeBitmask(View view) {
    ...
    } else if (view instanceof ViewGroup && isRCTButton(view, RCTRootViewReference.get())) {
  ...
  private static boolean isAdapterViewItem(View view) {
    ...
    Class nestedScrollingChildClass =
        getExistingClass("android.support.v4.view.NestedScrollingChild");
