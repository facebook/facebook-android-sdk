...
class AppEventsLoggerImpl {
  ...
  void logEvent(String eventName, double valueToSum, Bundle parameters) {
    logEvent(
        eventName, valueToSum, parameters, false, ActivityLifecycleTracker.getCurrentSessionGuid());
  }
  ...
  void logPurchase(
      BigDecimal purchaseAmount, Currency currency, Bundle parameters, boolean isImplicitlyLogged) {
    ...
    synchronized (staticLock) {
      if (!Utility.stringsEqualOrEmpty(pushNotificationsRegistrationId, registrationId)) {
        ...
        AppEventsLoggerImpl logger =
            new AppEventsLoggerImpl(FacebookSdk.getApplicationContext(), null, null);
        ...
        if (AppEventsLoggerImpl.getFlushBehavior() != FlushBehavior.EXPLICIT_ONLY) {
          logger.flush();
        }
      }
    }
  ...
  static void setInstallReferrer(String referrer) {
    ...
    SharedPreferences preferences =
        ctx.getSharedPreferences(APP_EVENT_PREFERENCES, Context.MODE_PRIVATE);
    if (referrer != null) {
  ...
  static String getInstallReferrer() {
    InstallReferrerUtil.tryUpdateReferrerInfo(
        new InstallReferrerUtil.Callback() {
  ...
  private static void initializeTimersIfNeeded() {
    ...
    final Runnable attributionRecheckRunnable =
        new Runnable() {
          @Override
          public void run() {
  ...
  void logEventImplicitly(String eventName, Double valueToSum, Bundle parameters) {
    logEvent(
        eventName, valueToSum, parameters, true, ActivityLifecycleTracker.getCurrentSessionGuid());
  }

  void logEventImplicitly(
  ...
  void logEvent(
    ...
    if (FetchedAppGateKeepersManager.getGateKeeperForKey(
        APP_EVENTS_KILLSWITCH, FacebookSdk.getApplicationId(), false)) {
      Logger.log(
          LoggingBehavior.APP_EVENTS,
          "AppEvents",
