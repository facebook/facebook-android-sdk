...
public class AutomaticAnalyticsLogger {
  ...
  public static void logActivityTimeSpentEvent(String activityName, long timeSpentInSeconds) {
    ...
    if (settings != null && settings.getAutomaticLoggingEnabled() && timeSpentInSeconds > 0) {
      ...
      logger.logEvent(Constants.AA_TIME_SPENT_EVENT_NAME, timeSpentInSeconds, params);
    }
  }
    ...
    boolean logAsSubs =
        isSubscription
            && FetchedAppGateKeepersManager.getGateKeeperForKey(
                APP_EVENTS_IF_AUTO_LOG_SUBS, FacebookSdk.getApplicationId(), false);

    if (logAsSubs) {
      String eventName =
          InAppPurchaseEventManager.hasFreeTrialPeirod(skuDetails)
    ...
    try {
      ...
      if (type.equals("subs")) {
        params.putCharSequence(
            ...
            Boolean.toString(purchaseJSON.optBoolean("autoRenewing", false)));
        params.putCharSequence(
